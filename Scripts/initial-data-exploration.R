...
hist_rain <- function(df, col1, col2, num_bins = 30) {
    ...
    col1_q <- enquo(col1)
    col2_q <- enquo(col2)
    col1_c <- as.character(substitute(col1))
    col2_c <- as.character(substitute(col2))
...
hist_rain(df_clean, Aug., May, num_bins = 40)
...
df_clean %>%
    ...
    group_by(Rainfall) %>%
    summarise(n = n()) %>%
    ggplot(aes(Rainfall, n)) +
    geom_col()


bar_histogram <- function(df, from) {
...
plot_heavy_rain_days(df_clean, 2013)
...
box_plot(df_clean, Aug., from = 2008)
...
scatter_plot <- function(df, col1, col2, title) {
    ...
    ggplot(df, aes(!!col1_q, !!col2_q)) +
        geom_point() +
        geom_smooth(se = FALSE, col = "red4", size = 1.5) +
        theme_minimal() +
        labs(title = title)
}
